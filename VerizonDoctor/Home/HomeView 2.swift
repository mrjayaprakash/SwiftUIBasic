...
class MultiTouchSyncViewModel11: ObservableObject {
    @Published var isTouched: [Bool] = [false, false]
    @Published var targetPoints: [CGPoint] = [CGPoint.zero, CGPoint.zero]
    @Published var testCompleted = false
    @Published var testResult: TestResult? = nil
    ...
    func evaluateTouches(_ detectedPoints: [CGPoint], in size: CGSize) {
        ...
        if isTouched.allSatisfy({ $0 }) {
            ...
            if currentRound >= requiredRounds {
                let duration = startTime.map { now.timeIntervalSince($0) }
                let firstRoundTime = roundCompletionTimes[0].timeIntervalSince(roundStartTimes[0])
                let secondRoundTime = roundCompletionTimes[1].timeIntervalSince(roundStartTimes[1])

                let testPassed = firstRoundPassed && secondRoundPassed
                let reason = testPassed ? "None" : (!firstRoundPassed ? "Missed first round" : "Missed second round")
                ...
                testResult = TestResult(
                    ...
                    duration: duration,
                    metadata: [
                        "multiTouchAccuracy": String(format: "%.2f", Double(passedRounds) / 2.0),
                        "firstRoundPassed": firstRoundPassed,
                        "secondRoundPassed": secondRoundPassed,
                        "firstRoundTimeout": firstRoundTime > roundTimeLimit,
                        "secondRoundTimeout": secondRoundTime > roundTimeLimit,
    ...
    private func startTimer() {
        timer?.invalidate()
        roundStartTimes.append(Date()) // 🕒 Track round start
...
//            let secondRoundTime = self.roundCompletionTimes[1].timeIntervalSince(self.roundStartTimes[1])
            ...
            if roundStartTimes.count > 1 && roundCompletionTimes.count > 1 {
                secondRoundTime = roundCompletionTimes[1].timeIntervalSince(roundStartTimes[1])
            } else {
                secondRoundTime = 10.0 // fallback for second round timeout
            }
